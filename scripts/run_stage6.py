...
def find_initiation_site(divfields: list[np.ndarray]) -> tuple[int, int] | None:
    """
    Returns (ix, iy) of the initiation site in pixel coordinates.

    Uses only the FIRST HALF of the pre-window (earliest, most localised
    frames) to avoid being dominated by the fully-spread peak contraction.
    Sums the negative divergence (contraction strength) over those frames
    and returns the pixel with maximum cumulative contraction.
    """
    if not divfields:
        return None
    n     = len(divfields)
    early = divfields[: max(1, n // 2)]   # first half = earliest frames
    cumsum = np.zeros_like(early[0])
    for d in early:
        cumsum += np.clip(-d, 0, None)    # only contraction (negative div)
    yx = np.unravel_index(np.argmax(cumsum), cumsum.shape)
    return int(yx[1]), int(yx[0])         # (x, y)
...
def analyze_pulses(
    ...
    for i, peak in enumerate(tqdm(peaks, desc="Analysing pulses", unit="pulse")):
        ...
        init_site = find_initiation_site(divfields)
...
def render_annotated_video(
    ...
    with tqdm(total=(total + stride - 1) // stride,
        ...
        while True:
            ...
            cv2.circle(frame, (round(cx), round(cy)), round(r),
